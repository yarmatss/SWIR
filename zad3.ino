...
char tryb = 'L'; // L - jasność (Light), H - wilgotność (Humidity), T - temperatura
...
// Poprawiona funkcja do wyświetlania literki trybu na lewym wyświetlaczu
void WyswietlTryb(char t) {
    switch(t) {
        case 'L': // Tryb jasności - litera L
            digitalWrite(2, HIGH); // a - wyłączony
            digitalWrite(3, LOW); // b - wyłączony
            digitalWrite(4, LOW); // c - wyłączony
            digitalWrite(5, HIGH);  // d - włączony
            digitalWrite(6, LOW);  // e - włączony
            digitalWrite(7, LOW);  // f - włączony
            digitalWrite(8, LOW); // g - wyłączony
            break;
        case 'H': // Tryb wilgotności - litera H
            digitalWrite(2, HIGH); // a - wyłączony
            digitalWrite(3, HIGH);  // b - włączony
            digitalWrite(4, HIGH);  // c - włączony
            digitalWrite(5, LOW); // d - wyłączony
            digitalWrite(6, LOW);  // e - włączony
            digitalWrite(7, LOW);  // f - włączony
            digitalWrite(8, LOW);  // g - włączony
            break;
        case 'T': // Tryb temperatury - litera t (mała)          
            digitalWrite(2, HIGH); // a - wyłączony
            digitalWrite(3, HIGH); // b - wyłączony
            digitalWrite(4, HIGH); // c - wyłączony
            digitalWrite(5, LOW);  // d - włączony
            digitalWrite(6, LOW);  // e - włączony
            digitalWrite(7, LOW);  // f - włączony
            digitalWrite(8, HIGH);  // g - włączony
            break;
    }
}

void setup() { 
    // Inicjalizacja czujnika DHT11 na pinie 12
    dht.setup(12); 
    ...
    pinMode(10, OUTPUT); // Kontrola wyświetlacza dziesiątek
    pinMode(11, OUTPUT); // Kontrola wyświetlacza jedności
    
    // Konfiguracja przycisku z INPUT_PULLUP na pinie D9
...
void loop() {
    ...
    // Jeśli nie wciśnięto przycisku, wyświetl drugą cyfrę
    if (digitalRead(9) == HIGH) {
        Znak((liczba / 10) % 10);
    } else {
        // Jeśli wciśnięto przycisk, wyświetl tryb
        WyswietlTryb(tryb);
        
        // Zmień tryb po zwolnieniu przycisku
        delay(200); // Debounce
    ...
    // Pomiar i aktualizacja wartości co około 200ms
    if (licznik++ > 100) {
        // Pomiar jasności
        ...
        // Pomiar wilgotności i temperatury z DHT11
        wilgotnosc = (int)(dht.getHumidity());
        temperatura = (int)(dht.getTemperature());
        
        // Ustaw odpowiednią wartość do wyświetlenia zgodnie z trybem
        switch(tryb) {
            case 'L': 
                liczba = jasnosc; 
                break;
